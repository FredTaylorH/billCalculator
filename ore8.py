...
#testBill = customerBills[0]
#for entry in testBill:
#	print(entry,testBill[entry])


		
#TODO: Reconsider implementation of NormalPositiveBill. Include weird crap?	

#print(cumulativeKwh)
	#calculateBill(bill['preSolarGrossKwh'],bill['preSolarSolarProduction'],bill['cumulativeCutoffsList'],bill['lengthInDays'],bill['allowancePerDay'],bill['nonCumulativeCutoffsList'],bill['tierPriceList'],bill['genRate'],bill['fixedCharges'],bill['ppa'])

	
	
	
#TODO:
#IMPLEMENT START AND END DATE FOR BILLPERIOD WE ARE INTERESTED IN
#Integrate billcalcs (oreBillCalc3) with this infrastructure
#Turn dummy datasets (~3 bills) into actual datasets with all 80 customers
#Sort list by startDate
#Get cumulative cutoffs	
#Do abs() to deal with negative billing
#True-up rules
#Deal with exports
...
'''
	#standardBillingDeterminants = (bill['preSolarGrossKwh'],bill['cumulativeCutoffsList'],bill['lengthInDays'],bill['allowancePerDay'],bill['nonCumulativeCutoffsList'],bill['tierPriceList'],bill['genRate'],bill['fixedCharges'])
	#print()
	#print(cumulativeKwh)
...
#for bill in customerBills:
#	print(bill['billStart'])

#preSolarBillDeterminants = 



#for usage,solarProduction in [(bill['postSolarNetKwh'],bill['postSolarSolarProduction']),(bill['preSolarGrossKwh'],bill['preSolarSolarProduction'])]:

#usage,solarProduction = 
...
for bill in customerBills: #Do one by one because order matters
	#cumulativeTotalKwh = 0	
	for usage,solarProduction in [(bill['postSolarNetKwh'],bill['postSolarSolarProduction']),(bill['preSolarGrossKwh'],bill['preSolarSolarProduction'])]:
	
		#print(usage)
		
		#print(usage,SolarProduction)
		#cumulativeTotalKwh = 0
	
		#cumulativeTotalKwh = 0	
		#print(bill['billStart'])
		
		absoluteValueKwh = abs(usage) #Convert to abs value first and THEN apply sign at end
		sign = (usage > 0) - (usage < 0)
		#cumulativeTotalKwh = cumulativeTotalKwh + usage
		#print(cumulativeTotalKwh)
		...
		#Get highest tier
		highestTier = 0 
		for cutoff in bill['cumulativeCutoffsList']:
			if (cutoff * bill['lengthInDays'] * bill['allowancePerDay']) <= absoluteValueKwh :
				highestTier+=1
		
		#Get kWh in all tiers, full and partially full
		fullTiersKwh = []
		for tier in range(1,highestTier):
			fullTiersKwh.append(bill['nonCumulativeCutoffsList'][tier] * bill['lengthInDays'] * bill['allowancePerDay'])
		partialTierKwh = absoluteValueKwh - sum(fullTiersKwh)
		
		#Get each bill component
		fullTiersBill = sum([a*b for a,b in zip(bill['tierPriceList'],fullTiersKwh)])
		partialTierBill = partialTierKwh * bill['tierPriceList'][highestTier-1]
		generationBill = (bill['genRate']*absoluteValueKwh)
		fixedChargeBill = (bill['lengthInDays']*bill['fixedCharges'])
		solarBill = (bill['ppa'] *  solarProduction)
		
		#Get total bill
		totalUtilityBill = fullTiersBill + partialTierBill + generationBill + fixedChargeBill + solarBill
		#print(totalUtilityBill)
...
'''
def lookup2(compoundKey,dictionariesToSearch,valuesToReturn,dictToUpdate,valuesToConvertToNumbers):
	...
	for valueToReturn in valuesToReturn:
		for eachDictionary in dictionariesToSearch:
			if compoundKey.items() <= eachDictionary.items():
				dictToUpdate[valueToReturn] = eachDictionary[valueToReturn]	
	for value in valuesToConvertToNumbers:
		value = float(value)
		
		
		
	#print('fullTiersBill is: ',fullTiersBill)
	#print('partialTierBill is: ',partialTierBill)
	#print('generationBill is: ',generationBill)
	#print('fixedChargeBill: ',fixedChargeBill)
	#solarBill = (ppa *  solarProduction)
	#print('solarBill is: ',solarBill)
	#print('Standard bill is ',totalUtilityBill)		
...
def readInCsv2(file,subsetField=None,subsetObservation=None):
	data = []
	with open(file) as f:
		for row in csv.DictReader(f, skipinitialspace=True): #Read in data, observation by observation
			observation = {key : value for key , value in row.items()}
			#print(observation)
			data.append(observation)
